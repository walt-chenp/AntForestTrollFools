...
- 收取记录面板：紧凑统计与四项收取控制；通过右上角“功能设置”管理定时收取和步数模拟；支持“清空日志”与“复制日志”。
...
- 自动赚能量（开发中）：手动进入并开启“赚能量”后，自动识别活动 Canvas 内出现的好友头像并点击；仅在活动专用页面生效，不影响森林首页收取。
...
- 收取日志：最新记录显示在顶部；保留开关状态、本轮扫描开始/结束、浇水赠能和成功收取结果，并区分自己与好友能量。
- 能量统计：今日显示 g；累计满 1,000 g 后按两位小数换算为 kg；收取成功回包会去重计入统计。

> v2.6 正式版：iOS 15 及以上使用 `AntForestPort-v2.6.dylib` 通用包（`arm64 + arm64e`）；iOS 14 请使用独立兼容包。
...
- 正式日志收敛：面板只展示用户可读的收取开始、结束、成功与开关状态；开发期诊断输出不进入正式日志。

## 下一版开发中

- 已合入自动赚能量：用户手动进入并开启活动后，插件通过活动 Canvas 自动点击出现的好友头像；结算后停止，不会在普通森林首页运行。
- 悬浮叶子优化：首页首次打开或从后台返回后完整显示；静置约 2 秒贴边缩小，不再因好友页、能量雨等页面切换反复弹出。
- 后台限制说明：退到桌面后不保证 H5/RPC 回包及实际收取；回到蚂蚁森林首页时会立即尝试补跑一次。

## iOS 14 独立版本

- iOS 14 请使用独立包 `AntForestPort-v2.6-iOS14.dylib`；它不替代 iOS 15+ 的 `AntForestPort-v2.6.dylib`，两个包均为 `arm64 + arm64e` 通用 dylib。
...
1. 使用巨魔注入器 TrollFools 注入 [`AntForestPort-v2.6.dylib`](build/AntForestPort-v2.6.dylib) 到支付宝。
...
7. 点击右上角齿轮进入“功能设置”：可管理多个每日固定收取时刻（点选修改、左滑删除），也可独立配置“步数模拟（测试）”。
...
1. 使用 TrollFools 同时向支付宝注入 `AntForestPort-v2.6.dylib` 和 `ImmortalizerJailed.dylib`。
...
| iOS | 常规包支持 iOS 15 及以上；iOS 14 使用独立 `AntForestPort-v2.6-iOS14.dylib` |
...
| 自动赚能量（开发中） | 已验证自动点击与正常结算；仅活动专用页面生效 |
...
```sh
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer make TARGET=build/AntForestPort-v2.6.dylib
```

本地 v2.6 产物位于 `build/AntForestPort-v2.6.dylib`。
...
- `release/v2.6`：正式维护线；已发布 v2.6 包含首页自动收取、本人优先、浇水赠能、步数模拟与叶子贴边，自动赚能量作为下一版开发内容。
